...
def test_user_prompt_injects_paper_position():
    ...
    part = _paper_position_part({"qty": 1000, "avg_cost": 10.0}, close=11.0, raw_close=11.0)
    ...
    prompt = _build_user_prompt(
        ...
        paper_position={"qty": 1000, "avg_cost": 10.0}, raw_close=11.0,
    )
    assert "模拟盘" in prompt and "持仓数量" in prompt
...
def test_paper_position_part_empty_without_position():
    ...
    assert _paper_position_part(None, 11.0, 11.0) == ""
